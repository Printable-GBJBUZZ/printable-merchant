...
import { Printer, User, Eye } from "lucide-react";
...
const OrderTable = () => {
	...
	const handleOrderUpdate = async (orderId: string, status: string) => {
		...
		try {
			const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_ROOT_URL}/api/order/${orderId}`, {
				method: "PATCH",
				headers: {
	...
	return (
      ...
      `}</style>
			...
			<table className="h-full min-w-full bg-white shadow-md rounded-lg overflow-hidden">
							...
							<tr key={index}>
								<td className="py-4 px-4 text-sm text-gray-600">
									<div className="flex items-center justify-between">
										<span className="truncate max-w-[150px]">{data.orderNo}</span>
										<button
									...
									{data.status === "pending" ? (
										<div className="flex gap-2">
											<button
												...
												onClick={() => handleOrderUpdate(data.orderNo, "Queued")}
												disabled={loadingStates[data.orderNo]?.isLoading}>
												Accept
												{loadingStates[data.orderNo]?.isLoading &&
													loadingStates[data.orderNo]?.newStatus === "Queued" && (
											...
											<button
												...
												onClick={() => handleOrderUpdate(data.orderNo, "Cancelled")}
												disabled={loadingStates[data.orderNo]?.isLoading}>
												Deny
												{loadingStates[data.orderNo]?.isLoading &&
													loadingStates[data.orderNo]?.newStatus === "cancelled" && (
								...
								<td className="py-4 px-4 text-sm text-gray-600">₹{data.amount}.00</td>
